...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (gamma, beta, eps, sample_mean, sample_var, x, x_hat, N)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    gamma, beta, eps, mean, var, x, x_hat, N = cache
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat*(x-mean)*(-1/2)*(var+eps)**(-3/2), axis=0)
    dmean = (np.sum(-dx_hat/np.sqrt(var+eps), axis=0)) + dvar*(np.sum(-2*(x-mean), axis=0))/N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # Convolution:
    for f in range(0, F):                 # Loop on the filters number
        h_i = 0
        for height in range(0, H_out):            # Loop on the hight of the image
            w_i = 0
            for width in range(0, W_out):
                out[:, f, height, width] = np.sum(x_pad[:, :, h_i:h_i+HH, w_i:w_i+WW] * w[f], axis=(1,2,3)) + b[f]
                w_i += stride
            h_i += stride
...
def max_pool_backward_naive(dout, cache):
    ...
    # Getting the data and some sizes:
    x, pool_param = cache
    pool_h = pool_param['pool_height']
    pool_w = pool_param['pool_width']
    stride = pool_param['stride']
    ...
    for n in range(N):
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    i, j = np.unravel_index(np.argmax(x[n, c, h*stride:h*stride+pool_h, w*stride:w*stride+pool_w]), (pool_h, pool_w))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # Old implamantaion:
    #for c in range(C):
    #    x_hat = x[:, c, :, :].reshape((N, H*W))
    #    gamma_hat = gamma[c] * np.ones((H*W,))
    #    beta_hat = beta[c] * np.ones((H*W,))
    #    out_hat, cache = batchnorm_forward(x_hat, gamma_hat, beta_hat, bn_param)
    #    out[:, c, :, :] = out_hat.reshape((N, H, W))
    
    # Reshape the input for Vanilla BN:
    x_hat = x.transpose((0, 2, 3, 1)).reshape((-1, C))
    
    # Calling Vanilla BN:
    out_hat, cache = batchnorm_forward(x_hat, gamma, beta, bn_param)
    
    # Reshaping back to (N, C, H, W):
    out = out_hat.reshape((N, H, W, C)).transpose((0, 3, 1, 2))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # Reshape the input to (N, D) for the Vanilla BN:
    dout_hat = dout.transpose((0, 2, 3, 1)).reshape((-1, C))
    
    # Calling Vanilla BN:
    dx_hat, dgamma, dbeta = batchnorm_backward(dout_hat, cache)
    
    # Reshaping dx back to (N, C, H, W):
    dx = dx_hat.reshape((N, H, W, C)).transpose((0, 3, 1, 2))
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # *** My implementation: ***
    # Getting the shapes and spliting x to the groups:
    #N, C, H, W = x.shape
    #x_hat = x.reshape((N, G, C//G, H, W))
    
    # Calculating mean an variance for correct axies:
    #mean = np.mean(x_hat, axis=(2, 3, 4), keepdims=True)
    #var = np.var(x_hat, axis=(2, 3, 4), keepdims=True)
    
    # Normalizing the input:
    #x_hat = (x_hat - mean) / np.sqrt(var + eps)
    
    # Reshaping back the output:
